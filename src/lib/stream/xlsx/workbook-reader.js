...
if(typeof require !== 'undefined') {
  ...
  try {
    ...
    _PassThrough = rs.PassThrough;
    _Readable = rs.Readable;
  } catch(e) {}
  try { _nodeStream = require('stream'); } catch(e) {}
  try { _unzip = require('unzipper'); } catch(e) {}
  try { _tmp = require('tmp'); } catch(e) {}
}
...
class WorkbookReader extends _EventEmitter {
  ...
  _getStream(input) {
    if (input instanceof _nodeStream.Readable || input instanceof _Readable) {
      return input;
    }
    if (typeof input === 'string') {
      return _fs.createReadStream(input);
    }
  ...
  async *parse(input, options) {
    ...
    const zip = _unzip.Parse({forceStream: true});
    stream.pipe(zip);
          ...
          if (entry.path.match(/xl\/worksheets\/sheet\d+[.]xml/)) {
            ...
            if (this.sharedStrings && this.workbookRels) {
              ...
              await new Promise((resolve, reject) => {
                _tmp.file((err, path, fd, tempFileCleanupCallback) => {
                  if (err) {
                    return reject(err);
                  }
                  waitingWorkSheets.push({sheetNo, path, tempFileCleanupCallback});

                  const tempStream = _fs.createWriteStream(path);
    ...
    for (const {sheetNo, path, tempFileCleanupCallback} of waitingWorkSheets) {
      let fileStream = _fs.createReadStream(path);
      // TODO: Remove once node v8 is deprecated
      // Detect and upgrade old fileStreams
      if (!fileStream[Symbol.asyncIterator]) {
        fileStream = fileStream.pipe(new _PassThrough());
      }
