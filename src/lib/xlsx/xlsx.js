...
if(typeof require !== 'undefined') {
  ...
  try { const rs = require('readable-stream'); _PassThrough = rs.PassThrough; } catch(e) {}
}
...
function fsReadFileAsync(filename, options) {
  return new Promise((resolve, reject) => {
    _fs.readFile(filename, options, (error, data) => {
      if (error) {
        reject(error);
      } else {
        resolve(data);
...
class XLSX {
  ...
  async read(stream, options) {
    ...
    if (!stream[Symbol.asyncIterator] && stream.pipe) {
      stream = stream.pipe(new _PassThrough());
    }
    const chunks = [];
    for await (const chunk of stream) {
      chunks.push(chunk);
    }
    return this.load(_Buffer.concat(chunks), options);
  }

  async load(data, options) {
    let buffer;
    if (options && options.base64) {
      buffer = _Buffer.from(data.toString(), 'base64');
    } else {
      buffer = data;
    }
